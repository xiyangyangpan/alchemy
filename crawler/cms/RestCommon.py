...
#     article
# --------------------------------------------------------------------
def rest_pub_article(article_type, url, rest_session_headers):
...
def pub_articles_to_site(rest_session_headers, domain, bulk_size=1):
    ...
    for article in articles:
        ...
        if (not article.publishedFlag) or (domain == 'all'):
            ret_post = rest_pub_article('article', article.url, rest_session_headers)
            if not ret_post:
